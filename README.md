...
## WifiSecurityTesting:
A pair of Python scripts (require scapy and a Linux machine with a monitor-mode/injection-capable Wi-Fi adapter) for testing the security of Wi-Fi networks you own or are explicitly authorized to test:
- **wifi_recon.py**: Passive scan. Listens for beacon/probe-response frames on a monitor-mode interface and reports nearby SSIDs, BSSIDs, channels, signal strength, and encryption type (Open/WEP/WPA/WPA2-3). Never transmits a packet. Run with `sudo python3 wifi_recon.py -i <mon-interface>`.
- **wifi_deauth_test.py**: Active deauthentication resilience test. Sends 802.11 deauth frames to a target AP/client to verify reconnection behavior, PMF/802.11w protections, or to force a handshake for your own offline password audit. This is disruptive and requires typing an explicit authorization confirmation before it will run (or passing `--yes-i-am-authorized` once you've reviewed the risks). Run with `sudo python3 wifi_deauth_test.py -i <mon-interface> -b <target-bssid>`.

Both scripts require a wireless adapter that supports monitor mode (most built-in laptop Wi-Fi chips do not); put it in monitor mode first with something like `sudo airmon-ng start wlan0`. Only ever point these at networks and clients you own or have explicit written authorization to test — unauthorized deauthentication attacks are illegal in most jurisdictions.
